...
#
# Per-task layout: <name-or-type> | <description> | <N tokens> | <duration>
...
while IFS= read -r ID; do
  ...
  # Prefer "type: name" when both are set and differ; else whichever is
  # present; else "unknown". Spawned Task-tool subagents have only .type
  # ("Explore"); the main row has only .name ("main").
  if [[ -n "$NAME" && -n "$TYPE" && "$NAME" != "$TYPE" ]]; then
    DISPLAY="${TYPE}: ${NAME}"
  elif [[ -n "$NAME" ]]; then
    DISPLAY="$NAME"
  elif [[ -n "$TYPE" ]]; then
  ...
  base=$(( 1 + 1 + ${#DISPLAY} + 9 + ${#tokens_fmt} + ${#duration_fmt} ))
  budget=$(( COLUMNS_AVAIL - base ))
  (( budget < MIN_DESC_BUDGET )) && budget=$MIN_DESC_BUDGET
  desc_trunc=$(truncate_str "$DESC" "$budget")

  body="${DIM}${DISPLAY}${RST}${SEP}${DIM}${desc_trunc}${RST}${SEP}${tok_color}${tokens_fmt}${RST}${SEP}${dur_color}${duration_fmt}${RST}"
